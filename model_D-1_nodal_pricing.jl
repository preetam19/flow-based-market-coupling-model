d_1_nodal_curt = zeros(Float64, length(T), length(N))
d_1_nodal_delta = zeros(Float64, length(T), length(N))
d_1_nodal_nod_inj = zeros(Float64, length(T), length(N))
d_1_nodal_line_f = zeros(Float64, length(T), length(L))
d_1_nodal_gen = zeros(Float64, length(T), length(P))
d_1_nodal_gen_costs = zeros(Float64, length(T), length(Z))
d_1_nodal_curt_costs = zeros(Float64, length(T), length(Z))
d_1_nodal_nodal_price = zeros(Float64, length(T), length(N))
d_1_nodal_np = zeros(Float64, length(T), length(Z_FBMC))
...
# Eq. (9p)
	...
	d_1_nodal_curt[Tsub,:] = JuMP.value.(CURT[:,:])
	d_1_nodal_delta[Tsub,:] = JuMP.value.(DELTA[:,:])
	d_1_nodal_nod_inj[Tsub,:] = JuMP.value.(NOD_INJ[:,:])
	d_1_nodal_line_f[Tsub,:] = JuMP.value.(LINE_F[:,:])
	d_1_nodal_gen[Tsub,:] = JuMP.value.(GEN[:,:])
	d_1_nodal_gen_costs[Tsub,:] = JuMP.value.(GEN_COSTS[:,:])
	d_1_nodal_curt_costs[Tsub,:] = JuMP.value.(CURT_COSTS[:,:])
	d_1_nodal_np[Tsub,:] = JuMP.value.(NP[:,:])
	d_1_nodal_nodal_price[Tsub,:] = dual.(nodal_balance[:,:])

	m = nothing
end

include("export_D-1_nodal_pricing_results.jl")
